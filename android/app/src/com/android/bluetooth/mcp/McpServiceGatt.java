 ...
 */
public class McpServiceGatt implements McpService {
    ...
    private PlaybackState mCurrentPlaybackState = PlaybackState.INACTIVE;
    ...
    private static class GattOpContext {
        ...
        public GattOpContext(Operation operation, int requestId,
    ...
    private final HashMap<Integer,
            McsCharacteristicWriteCallbacks> mAllMcsCharacteristicsWriteCallbacks =
            new HashMap<Integer, McsCharacteristicWriteCallbacks>() {
                {
                    put(McsCharactertisticId.TRACK_POSITION,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.PLAYBACK_SPEED,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.CURRENT_TRACK_OBJ_ID,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.NEXT_TRACK_OBJ_ID,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.CURRENT_GROUP_OBJ_ID,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.PLAYING_ORDER,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                                ...
                                if (value.length == 1
                                    ...
                                    BluetoothGattCharacteristic supportedPlayingOrderChar =
                                            mCharacteristics.get(
                                                    McsCharactertisticId.PLAYING_ORDER_SUPPORTED);
                    ...
                    put(McsCharactertisticId.MEDIA_CONTROL_POINT,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                    ...
                    put(McsCharactertisticId.SEARCH_CONTROL_POINT,
                            (device, requestId, characteristic, preparedWrite, responseNeeded,
                                    offset, value) -> {
                                if (VDBG)
                                ...
                                if (responseNeeded)
                                    mBluetoothGattServer.sendResponse(
                                            device, requestId, 0, offset, value);
                            });
                }
            };
    ...
    private int getDeviceAuthorization(BluetoothDevice device) {
        return mMcpServiceManager.getDeviceAuthorization(device);
    }
    ...
    private void onUnauthorizedGattOperation(BluetoothDevice device, GattOpContext op) {
        ...
        if (operations.size() == 1)
            mMcpServiceManager.onDeviceUnauthorized(device);
    }
        ...
        switch (op.mOperation) {
            case READ_CHARACTERISTIC:
                ...
                if (op.mCharacteristic.getUuid().equals(
                            mCharacteristics.get(McsCharactertisticId.TRACK_POSITION).getUuid())) {
            ...
            case WRITE_CHARACTERISTIC:
                if (op.mPreparedWrite) {
                    ...
                    int charId = sAllMcsCharDescriptors.get(op.mCharacteristic.getUuid()).id;
                    mAllMcsCharacteristicsWriteCallbacks.get(charId).onCharacteristicWriteRequest(
                ...
                if (op.mResponseNeeded) {
                    ...
                    if (op.mCharacteristic.getUuid().equals(UUID_MEDIA_CONTROL_POINT)) {
                        setMediaControlRequestResult(
                                new McpServiceMediaControlRequest(op.mValue[0], 0),
                        ...
                        setSearchRequestResult(null, McpServiceSearchRequest.Results.FAILURE, 0);
                    }
                }
                break;
            case WRITE_DESCRIPTOR:
    ...
    final BluetoothGattServerCallback mServerCallback = new BluetoothGattServerCallback() {
        ...
        public void onServiceAdded(int status, BluetoothGattService service) {
            ...
            mCharacteristics.get(McsCharactertisticId.CONTENT_CONTROL_ID)
        ...
        public void onCharacteristicReadRequest(BluetoothDevice device, int requestId, int offset,
            ...
            if (VDBG)
                Log.d(TAG,
                        "BluetoothGattServerCallback: "
                                + "onCharacteristicReadRequest offset= " + offset
                                + " entire value= " + characteristic.getValue());
    ...
    private void setInitialCharacteristicValues(boolean notify) {
        updateMediaStateChar(mCurrentPlaybackState.getValue());
        ...
        updateSupportedOpcodesChar(McpServiceMediaControlRequest.SupportedOpcodes.NONE, notify);
    }

    private void setInitialCharacteristicValues() { setInitialCharacteristicValues(false); }

    private void setInitialCharacteristicValuesAndNotify() { setInitialCharacteristicValues(true); }
    ...
    public class BluetoothGattServerProxy {
        ...
        public void close() { mBluetoothGattServer.close(); }
    ...
    protected McpServiceGatt(Context context, @NonNull ServiceCallbacks callbacks, int ccid) {
        mContext = context;
        mCallbacks = callbacks;
        mCcid = ccid;

        mMcpServiceManager = new ServiceFactory().getMcpServiceManager();
    };

    protected boolean init(UUID svc_uuid) {
        ...
        return initGattService(svc_uuid);
    }
    ...
    private static int getMediaControlPointRequestPayloadLength(int opcode) {
        switch (opcode) {
            ...
            case McpServiceMediaControlRequest.Opcodes.GOTO_SEGMENT:
            case McpServiceMediaControlRequest.Opcodes.GOTO_TRACK:
            case McpServiceMediaControlRequest.Opcodes.GOTO_GROUP:
                return 4;
            default:
                return 0;
        }
    ...
    int handleMediaControlPointRequest(byte[] value) {
        ...
        if (!isOpcodeSupported(opcode)) {
            ...
            mContext.getMainThreadHandler().post(() -> {
                setMediaControlRequestResult(new McpServiceMediaControlRequest(opcode, 0),
                        McpServiceMediaControlRequest.Results.OPCODE_NOT_SUPPORTED);
            });
        ...
        int intVal = 0;
        if (opcode == McpServiceMediaControlRequest.Opcodes.MOVE_RELATIVE
                || opcode == McpServiceMediaControlRequest.Opcodes.GOTO_SEGMENT
                || opcode == McpServiceMediaControlRequest.Opcodes.GOTO_TRACK
                || opcode == McpServiceMediaControlRequest.Opcodes.GOTO_GROUP) {
        ...
        McpServiceMediaControlRequest req = new McpServiceMediaControlRequest(opcode, intVal);
    ...
    public void setCallbacks(ServiceCallbacks callbacks) { mCallbacks = callbacks; }

    @VisibleForTesting
    protected void setServiceManagerForTesting(McpServiceManager manager) {
        mMcpServiceManager = manager;
    }
    ...
    private boolean initGattService(UUID serviceUuid) {
        ...
        for (Map.Entry<UUID, McsCharacteristicDescriptorData> entry :
                sAllMcsCharDescriptors.entrySet()) {
            McsCharacteristicDescriptorData desc = entry.getValue();
            ...
            if ((mFeatures & desc.featureFlag) != 0) {
                ...
                if ((myChar.getProperties() & BluetoothGattCharacteristic.PROPERTY_NOTIFY) != 0) {
    ...
    public void updatePlaybackState(PlaybackState state) {
        ...
        if ((state.getValue() <= PlaybackState.STATE_MAX.getValue())
                && (state.getValue() >= PlaybackState.STATE_MIN.getValue())) {
    ...
    private int getMediaStateChar() {
        if (isFeatureSupported(ServiceFeature.MEDIA_STATE)) {
            BluetoothGattCharacteristic stateChar =
                    mCharacteristics.get(McsCharactertisticId.MEDIA_STATE);
            return stateChar.getIntValue(BluetoothGattCharacteristic.FORMAT_UINT8, 0);
        }

        return PlaybackState.INACTIVE.getValue();
    }
    ...
    private void updateObjectID(int objectIdField, long objectIdValue, boolean notify) {
        ...
        if (isFeatureSupported(feature)) {
            updateObjectIdChar(mCharacteristics.get(McsCharactertisticId.FromFeature(feature)),
    ...
    @Override
    public void setMediaControlRequestResult(McpServiceMediaControlRequest request,
            McpServiceMediaControlRequest.Results resultStatus) {
        if (DBG)
            Log.d(TAG, "setMediaControlRequestResult");

        if (getMediaStateChar() == PlaybackState.INACTIVE.getValue()) {
            resultStatus = McpServiceMediaControlRequest.Results.MEDIA_PLAYER_INACTIVE;
        }
        ...
        BluetoothGattCharacteristic characteristic =
                mCharacteristics.get(McsCharactertisticId.MEDIA_CONTROL_POINT);
        characteristic.setValue(bb.array());
        notifyCharacteristic(characteristic, null);
    }

    @Override
    public void setSearchRequestResult(McpServiceSearchRequest request,
            McpServiceSearchRequest.Results resultStatus, long resultObjectId) {
        if (DBG)
            Log.d(TAG, "setSearchRequestResult");

        // TODO: There is no Object Trasfer Service implementation.
        BluetoothGattCharacteristic characteristic =
                mCharacteristics.get(McsCharactertisticId.SEARCH_CONTROL_POINT);
    ...
    public void updatePlayerState(Map stateFields) {
        ...
        if (stateFields.containsKey(PlayerStateField.PLAYBACK_STATE)) {
            PlaybackState blaybackState =
                    (PlaybackState) stateFields.get(PlayerStateField.PLAYBACK_STATE);
            if (DBG)
                Log.d(TAG,
                        "updatePlayerState: playbackState= "
                                + stateFields.get(PlayerStateField.PLAYBACK_STATE));

            if (blaybackState == PlaybackState.INACTIVE)
        ...
        if (mediaState != mCurrentPlaybackState.getValue()) {
            if ((getTrackDurationChar() == 0
                        || getTrackDurationChar() == TRACK_DURATION_UNAVAILABLE)) {
                // Set to INACTIVE if not set already
                if (mediaState != PlaybackState.INACTIVE.getValue()) {
                    updateMediaStateChar(PlaybackState.INACTIVE.getValue());
                }
            } else {
                updateMediaStateChar(mCurrentPlaybackState.getValue());
            }
        ...
        if (stateFields.containsKey(PlayerStateField.SEEKING_SPEED)) {
            ...
            if (playbackState == PlaybackState.SEEKING.getValue()) {
        ...
        if (notifyTrackChange) {
            if (isFeatureSupported(ServiceFeature.TRACK_CHANGED)) {
                BluetoothGattCharacteristic myChar =
                        mCharacteristics.get(McsCharactertisticId.TRACK_CHANGED);
    ...
    void updatePlayingOrderChar(PlayingOrder order, boolean notify) {
        ...
        if (isFeatureSupported(ServiceFeature.PLAYING_ORDER)) {
            BluetoothGattCharacteristic orderChar =
                    mCharacteristics.get(McsCharactertisticId.PLAYING_ORDER);
    ...
    private void notifyCharacteristic(@NonNull BluetoothGattCharacteristic characteristic,
        ...
        for (BluetoothDevice device : mBluetoothGattServer.getConnectedDevices()) {
            ...
            if (charCccMap != null) {
                ...
                if (VDBG)
                    Log.d(TAG,
                            "notifyCharacteristic char= " + characteristic.getUuid().toString()
                                    + " cccVal= "
                                    + ByteBuffer.wrap(ccc)
                                              .order(ByteOrder.LITTLE_ENDIAN)
                                              .getShort());
                if (Arrays.equals(ccc, BluetoothGattDescriptor.ENABLE_NOTIFICATION_VALUE)) {
                    if (VDBG)
    ...
    Float getSeekingSpeedChar() {
        ...
        if (isFeatureSupported(ServiceFeature.SEEKING_SPEED)) {
            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.SEEKING_SPEED);
            if (characteristic.getValue() != null) {
                Integer intVal =
    ...
    Float getPlaybackSpeedChar() {
        ...
        if (isFeatureSupported(ServiceFeature.PLAYBACK_SPEED)) {
            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.PLAYBACK_SPEED);
            if (characteristic.getValue() != null) {
                Integer intVal =
                        characteristic.getIntValue(BluetoothGattCharacteristic.FORMAT_SINT8, 0);
                speed = CharacteristicSpeedIntValueToSpeedFloat(intVal);
    ...
    @VisibleForTesting
    void updateTrackPositionChar(long positionMs, boolean force_notify) {
        if (VDBG)
            Log.d(TAG, "updateTrackPositionChar: " + positionMs);
        if (isFeatureSupported(ServiceFeature.TRACK_POSITION)) {
            final int position = (positionMs != TRACK_POSITION_UNAVAILABLE)
                    ? new Long(MillisecondsToMcsInterval(positionMs)).intValue()
                    : INTERVAL_UNAVAILABLE;

            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.TRACK_POSITION);
            ...
            if (isFeatureSupported(ServiceFeature.TRACK_POSITION_NOTIFY)) {
                ...
                if ((getMediaStateChar() == PlaybackState.PAUSED.getValue())
                        || (getMediaStateChar() == PlaybackState.SEEKING.getValue())
                        || force_notify)
                    notifyCharacteristic(characteristic, null);
            }
        }
    ...
    private long getTrackDurationChar() {
        if (isFeatureSupported(ServiceFeature.TRACK_DURATION)) {
            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.TRACK_DURATION);
            if (characteristic.getValue() != null) {
                int duration =
    ...
    private String getTrackTitleChar() {
        if (isFeatureSupported(ServiceFeature.TRACK_TITLE)) {
            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.TRACK_TITLE);
            if (characteristic.getValue() != null) {
    ...
    void updateSupportedOpcodesChar(int opcodes, boolean notify) {
        ...
        if (isFeatureSupported(ServiceFeature.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED)) {
            if (mCharacteristics.get(McsCharactertisticId.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED)
                            .getValue()
                    != null)
                // Do nothing if nothing has changed
                if (opcodes
                        == mCharacteristics
                                   .get(McsCharactertisticId.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED)
            ...
            BluetoothGattCharacteristic characteristic = mCharacteristics.get(
                    McsCharactertisticId.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED);
    ...
    void updatePlayingOrderSupportedChar(int supportedOrder) {
        ...
        if (isFeatureSupported(ServiceFeature.PLAYING_ORDER_SUPPORTED))
            mCharacteristics.get(McsCharactertisticId.PLAYING_ORDER_SUPPORTED)
                    .setValue(supportedOrder, BluetoothGattCharacteristic.FORMAT_UINT16, 0);
    }

    private void updateIconObjIdChar(Long objId) {
        if (isFeatureSupported(ServiceFeature.PLAYER_ICON_OBJ_ID))
            updateObjectIdChar(mCharacteristics.get(McsCharactertisticId.PLAYER_ICON_OBJ_ID), objId,
                    null, true);
    }
    ...
    private void updatePlayerIconUrlChar(String url) {
        ...
        if (isFeatureSupported(ServiceFeature.PLAYER_ICON_URL))
            mCharacteristics.get(McsCharactertisticId.PLAYER_ICON_URL).setValue(url);
    }

    private String getPlayerNameChar() {
        if (isFeatureSupported(ServiceFeature.PLAYER_NAME)) {
            BluetoothGattCharacteristic characteristic =
                    mCharacteristics.get(McsCharactertisticId.PLAYER_NAME);
            if (characteristic.getValue() != null)
                return characteristic.getStringValue(0);
        }
        return null;
    ...
    boolean isOpcodeSupported(int opcode) {
        if (opcode < McpServiceMediaControlRequest.Opcodes.PLAY
                || opcode > McpServiceMediaControlRequest.Opcodes.GOTO_GROUP)
            return false;

        if (isFeatureSupported(ServiceFeature.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED)) {
            Integer opcodeSupportBit = sOpcodeSupportMap.get(opcode);
            if (opcodeSupportBit != null)
                return (mCharacteristics
                                       .get(McsCharactertisticId
    ...
    private interface McsCharacteristicWriteCallbacks {
        void onCharacteristicWriteRequest(BluetoothDevice device, int requestId,
                BluetoothGattCharacteristic characteristic, boolean preparedWrite,
                boolean responseNeeded, int offset, byte[] value);
    }

    private static final class McsCharacteristicDescriptorData {
        public int id;
        public int properties;
        public int permissions;
        public long featureFlag;
        public long ntfFeatureFlag;

        private McsCharacteristicDescriptorData(
    ...
    private final static class McsCharactertisticId {
        ...
        public static int FromFeature(long feature) { return Long.numberOfTrailingZeros(feature); }
    }

    private static final UUID UUID_CCCD = UUID.fromString("00002902-0000-1000-8000-00805f9b34fb");

    /* Helper struct to map opcodes which are written to 'Media Control Point' characteristics to
     * their corresponding feature bit masks used in 'Media Control Point Opcodes Supported'
     * characteristic.
     */
    private final static HashMap<Integer, Integer> sOpcodeSupportMap =
            new HashMap<Integer, Integer>() {
                {
                    put(McpServiceMediaControlRequest.Opcodes.PLAY,
                            McpServiceMediaControlRequest.SupportedOpcodes.PLAY);
                    put(McpServiceMediaControlRequest.Opcodes.PAUSE,
                            McpServiceMediaControlRequest.SupportedOpcodes.PAUSE);
                    put(McpServiceMediaControlRequest.Opcodes.FAST_REWIND,
                            McpServiceMediaControlRequest.SupportedOpcodes.FAST_REWIND);
                    put(McpServiceMediaControlRequest.Opcodes.FAST_FORWARD,
                            McpServiceMediaControlRequest.SupportedOpcodes.FAST_FORWARD);
                    put(McpServiceMediaControlRequest.Opcodes.STOP,
                            McpServiceMediaControlRequest.SupportedOpcodes.STOP);
                    put(McpServiceMediaControlRequest.Opcodes.MOVE_RELATIVE,
                            McpServiceMediaControlRequest.SupportedOpcodes.MOVE_RELATIVE);
                    put(McpServiceMediaControlRequest.Opcodes.PREVIOUS_SEGMENT,
                            McpServiceMediaControlRequest.SupportedOpcodes.PREVIOUS_SEGMENT);
                    put(McpServiceMediaControlRequest.Opcodes.NEXT_SEGMENT,
                            McpServiceMediaControlRequest.SupportedOpcodes.NEXT_SEGMENT);
                    put(McpServiceMediaControlRequest.Opcodes.FIRST_SEGMENT,
                            McpServiceMediaControlRequest.SupportedOpcodes.FIRST_SEGMENT);
                    put(McpServiceMediaControlRequest.Opcodes.LAST_SEGMENT,
                            McpServiceMediaControlRequest.SupportedOpcodes.LAST_SEGMENT);
                    put(McpServiceMediaControlRequest.Opcodes.GOTO_SEGMENT,
                            McpServiceMediaControlRequest.SupportedOpcodes.GOTO_SEGMENT);
                    put(McpServiceMediaControlRequest.Opcodes.PREVIOUS_TRACK,
                            McpServiceMediaControlRequest.SupportedOpcodes.PREVIOUS_TRACK);
                    put(McpServiceMediaControlRequest.Opcodes.NEXT_TRACK,
                            McpServiceMediaControlRequest.SupportedOpcodes.NEXT_TRACK);
                    put(McpServiceMediaControlRequest.Opcodes.FIRST_TRACK,
                            McpServiceMediaControlRequest.SupportedOpcodes.FIRST_TRACK);
                    put(McpServiceMediaControlRequest.Opcodes.LAST_TRACK,
                            McpServiceMediaControlRequest.SupportedOpcodes.LAST_TRACK);
                    put(McpServiceMediaControlRequest.Opcodes.GOTO_TRACK,
                            McpServiceMediaControlRequest.SupportedOpcodes.GOTO_TRACK);
                    put(McpServiceMediaControlRequest.Opcodes.PREVIOUS_GROUP,
                            McpServiceMediaControlRequest.SupportedOpcodes.PREVIOUS_GROUP);
                    put(McpServiceMediaControlRequest.Opcodes.NEXT_GROUP,
                            McpServiceMediaControlRequest.SupportedOpcodes.NEXT_GROUP);
                    put(McpServiceMediaControlRequest.Opcodes.FIRST_GROUP,
                            McpServiceMediaControlRequest.SupportedOpcodes.FIRST_GROUP);
                    put(McpServiceMediaControlRequest.Opcodes.LAST_GROUP,
                            McpServiceMediaControlRequest.SupportedOpcodes.LAST_GROUP);
                    put(McpServiceMediaControlRequest.Opcodes.GOTO_GROUP,
     ...
     * them) This is set according to the Media Control Service Specification, Rev. 09r06.
     */
    private final static Map<UUID,
            McsCharacteristicDescriptorData> sAllMcsCharDescriptors = new HashMap<UUID,
            McsCharacteristicDescriptorData>() {
        {
            put(UUID_PLAYER_NAME,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PLAYER_NAME,
                            ServiceFeature.PLAYER_NAME, ServiceFeature.PLAYER_NAME_NOTIFY,
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_PLAYER_ICON_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PLAYER_ICON_OBJ_ID,
                            ServiceFeature.PLAYER_ICON_OBJ_ID,
                            // Notifications unsupported
                            0, BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_PLAYER_ICON_URL,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PLAYER_ICON_URL,
                            ServiceFeature.PLAYER_ICON_URL,
                            // Notifications unsupported
                            0, BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_TRACK_CHANGED,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.TRACK_CHANGED,
                            ServiceFeature.TRACK_CHANGED,
                            // Mandatory notification if char. exists.
                            ServiceFeature.TRACK_CHANGED,
                            BluetoothGattCharacteristic.PROPERTY_NOTIFY, 0));
            put(UUID_TRACK_TITLE,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.TRACK_TITLE,
                            ServiceFeature.TRACK_TITLE, ServiceFeature.TRACK_TITLE_NOTIFY,
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_TRACK_DURATION,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.TRACK_DURATION,
                            ServiceFeature.TRACK_DURATION, ServiceFeature.TRACK_DURATION_NOTIFY,
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_TRACK_POSITION,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.TRACK_POSITION,
            ...
            put(UUID_PLAYBACK_SPEED,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PLAYBACK_SPEED,
                            ...
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED
                                    | BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_SEEKING_SPEED,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.SEEKING_SPEED,
                            ServiceFeature.SEEKING_SPEED, ServiceFeature.SEEKING_SPEED_NOTIFY,
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_CURRENT_TRACK_SEGMENT_OBJ_ID,
                    new McsCharacteristicDescriptorData(
                            McsCharactertisticId.CURRENT_TRACK_SEGMENT_OBJ_ID,
                            ServiceFeature.CURRENT_TRACK_SEGMENT_OBJ_ID,
                            // Notifications unsupported
                            0, BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_CURRENT_TRACK_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.CURRENT_TRACK_OBJ_ID,
                            ...
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED
                                    | BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_NEXT_TRACK_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.NEXT_TRACK_OBJ_ID,
                            ...
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED
                                    | BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_CURRENT_GROUP_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.CURRENT_GROUP_OBJ_ID,
                            ...
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED
                                    | BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_PARENT_GROUP_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PARENT_GROUP_OBJ_ID,
                            ServiceFeature.PARENT_GROUP_OBJ_ID,
                            ServiceFeature.PARENT_GROUP_OBJ_ID_NOTIFY,
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_PLAYING_ORDER,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.PLAYING_ORDER,
                            ...
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED
                                    | BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_PLAYING_ORDER_SUPPORTED,
                    new McsCharacteristicDescriptorData(
                            McsCharactertisticId.PLAYING_ORDER_SUPPORTED,
                            ...
                            0, BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_MEDIA_STATE,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.MEDIA_STATE,
                            ...
                            BluetoothGattCharacteristic.PROPERTY_READ
                                    | BluetoothGattCharacteristic.PROPERTY_NOTIFY,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_MEDIA_CONTROL_POINT,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.MEDIA_CONTROL_POINT,
                            ...
                            BluetoothGattCharacteristic.PROPERTY_WRITE
                                    ...
                                    | BluetoothGattCharacteristic.PROPERTY_NOTIFY,
                            BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_MEDIA_CONTROL_POINT_OPCODES_SUPPORTED,
                    new McsCharacteristicDescriptorData(
                            McsCharactertisticId.MEDIA_CONTROL_POINT_OPCODES_SUPPORTED,
                            ...
                            BluetoothGattCharacteristic.PROPERTY_READ,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_SEARCH_RESULT_OBJ_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.SEARCH_RESULT_OBJ_ID,
                            ...
                            BluetoothGattCharacteristic.PROPERTY_READ
                                    | BluetoothGattCharacteristic.PROPERTY_NOTIFY,
                            BluetoothGattCharacteristic.PERMISSION_READ_ENCRYPTED));
            put(UUID_SEARCH_CONTROL_POINT,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.SEARCH_CONTROL_POINT,
                            ...
                            BluetoothGattCharacteristic.PROPERTY_WRITE
                                    ...
                                    | BluetoothGattCharacteristic.PROPERTY_NOTIFY,
                            BluetoothGattCharacteristic.PERMISSION_WRITE_ENCRYPTED));
            put(UUID_CONTENT_CONTROL_ID,
                    new McsCharacteristicDescriptorData(McsCharactertisticId.CONTENT_CONTROL_ID,
    ...
    public void dump(StringBuilder sb) {
        ...
        sb.append("\n\t\tCurrentPlaybackState = " + mCurrentPlaybackState);
    }
}
